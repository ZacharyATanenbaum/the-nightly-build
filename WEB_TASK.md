...
- GitHub Actions are deterministic infrastructure only: independent proof,
  browser render validation, protected auto-merge, and static publishing.
- Git is durable memory. Reconstruct state from `main`, `library`, article PRs,
  bot comments, check runs, publisher runs, and the published endpoint every
  time.
...
3. The repository's sole configured publisher has completed successfully after
   the merge.
4. The exact article exists at the published endpoint with matching series,
   slug, title, and date metadata.

A green model claim, a PR URL, a successful validator without a merge, or a green
publisher without the expected article is not completion.
...
2. Inspect open and recently merged article PRs before commissioning work.
   ...
   - Treat the latest check runs, job logs, and bot-authored editor findings as
     machine evidence.
...
8. Record the PR number, current head SHA, article path, and current published
   revision, then enter the closure and repair loop. Do not stop after opening
   the PR.
...
6. After merge, wait for the sole configured publisher. Inspect and rerun a
   transient publisher failure within the same two-rerun bound.
7. Verify publication independently. The published revision must advance and the
   exact article path must contain matching series, slug, title, and date
   metadata. A green publisher without the article is a failure.
8. If the bounded repair or rerun allowance is exhausted, preserve the existing
...
4. Remove any previously installed GitHub Models generation workflow or cron.
   ...
   exactly one static publisher.
5. Test the route with a disposable or historical article on an isolated base:
   require validator success, a real browser-render pass, auto-merge, publisher
   success, and exact published metadata before declaring setup complete.
6. Run the real task once when the product exposes that control and verify the
