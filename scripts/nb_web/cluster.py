...
STOPWORDS = frozenset(
    """
    about after again against all also amid among and announces announcement
    are artificial as at before being between both but by can company companies
    could data from for has have how into intelligence its launch launches made
    make makes making model models more most new news not now of on one or our
    over paper platform platforms release releases research says software system
    systems than that the their them there these they this through to tool tools
    under up update updates use uses using via was were what when where which who
    why will with would
    """.split()
)
